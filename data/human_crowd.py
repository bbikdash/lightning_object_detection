...
The CrowdHuman dataset stores its annotations in a variant of the json file format. Part of the annotation
loading process can be performed like this: https://gist.github.com/Mycenae/d29f5402dd5f8be76d45e1909d5aa4e3
"""
...
class HumanCrowdDataset(Dataset):
    def __init__(self,
                 root: str,
                 split: str,
        ...
        self.image_dir = os.path.join(root, f"{split}")
        annotation_file = os.path.join(root, f"annotation_{split}.odgt")
        ...
        self.name = "human_crowd_dataset"

        self.annotations = self._process_annotations(annotation_file)
    ...
    def _process_annotations(self, annotation_file):
        """
        annotations: dict, key,value => [img_file_path (str): bboxes (np.ndarray)]
        bboxes is an N x 5 np.ndarray with the following structure: [class_id, xcent, ycent, width, height]
        """
        annotations = {}
        try:
            with open(annotation_file, 'r') as f:
                for line in f:
                    ...
                    img_file_path = os.path.join(self.image_dir, f"{data['ID']}.jpg")
                    bboxes = np.zeros((1,5))

                    for gtbox in data['gtboxes']:
                        tag = gtbox['tag']
                        if tag == 'person':
                            bbox = np.hstack(([self.SOURCE_CLASS_BIMAP.inverse[tag]], gtbox['vbox']))
                            bboxes = np.vstack((bboxes, bbox))
                    bboxes = np.delete(bboxes, 0, axis=0)
                    
                    # Convert COCO to unnormalized YOLO format. Basically, move the bbox down and right
                    bboxes[:,1:] = xyxyCOCO2cxcywh(bboxes[:,1:])
                    annotations[img_file_path] = bboxes
    ...
    def load_annotation(self, index: int) -> np.ndarray:
        ...
        # Get the name of the image
        k = list(self.annotations.keys())[index]
        label_data = self.annotations[k].copy()  # [class_id, x_center, y_center, width, height]
    ...
    def _apply_augmentations(self, image: np.ndarray, target: np.ndarray) -> Tuple[torch.Tensor, np.ndarray]:
        ...
        Args:
            ...
            target: Nx5 numpy array [class_id, x_center, y_center, width, height] (unnormalized)
...
if __name__ == "__main__":
    ...
    dataset = HumanCrowdDataset(root="/mnt/data/Datasets/Person_Detection/CrowdHuman",
