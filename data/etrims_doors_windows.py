...
with the original bounding box labels in the following format: [class_id, x_min, y_min, width, height]
"""
...
class ETrimsDoorsWindowsDataset(Dataset):
    def __init__(self,
                 root: str,
                 split: str,
        ...
        """
        self.root = root
        self.image_dir = os.path.join(root, f"images/08_etrims-ds/{split}")
        self.annotation_dir = os.path.join(root, f"annotations/08_etrims-ds/{split}")
        ...
        self.name = "etrims_window_doors_dataset"

        # A set of composed Albumentation transformations (can be for training or validation)
        new_transforms = [A.CropAndPad(percent=(0.0, 1.75), p=0.5)]
        for t in transforms:
            if isinstance(t, A.RandomResizedCrop):
                ...
                t.shift_limit = (-0.15, 0.15)
                t.scale_limit = (1.0, 1.0)
                t.rotate_limit = (-30, 30)
...
if __name__ == "__main__":
    ...
    transforms = A.Compose([
        ...
        # A.CropAndPad(percent=(1.0, 1.5), p=0.5),
        ...
        A.ShiftScaleRotate(shift_limit=0.15, scale_limit=0.01, rotate_limit=30),
    ...
    dataset = ETrimsDoorsWindowsDataset(root="/mnt/data/Datasets/Doors_Windows/etrims-db_v1",
