...
class SoccerPlayerDataset(Dataset):
    def __init__(self,
                 root: str,
                 split: str,
        ...
        self.image_dir = os.path.join(root, f"images/{split}")
        self.annotation_dir = os.path.join(root, f"annotations/{split}")
        ...
        new_transforms = []
        for t in transforms:
            if isinstance(t, A.RandomRotate90):
                t.p = 0.00001
    ...
    def load_annotation(self, index: int) -> np.ndarray:
        ...
        if len(label_data) == 0:
            ...
            label_data[:,1:] = xyxy2cxcywh(label_data[:,1:])
...
if __name__ == "__main__":
    ...
    dataset = SoccerPlayerDataset(root="/mnt/data/Datasets/Person_Detection/Soccer_Player_Dataset",
